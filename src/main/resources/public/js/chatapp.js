...
function updateAppStatusArea(message, isErr){
...
function updateChatRoom(message) {
    ...
    updateAppStatusArea("Received response from Server - type [" + type + "]", false);
...
function nullHandler(message){
    updateAppStatusArea(JSON.parse(message).message, true);
...
function userRoomsHandler(message){
    ...
    //     clearChatAndUsers();
    //     updateAppStatusArea("Room " + currentRoomName + " no longer exists", true);
    // }
...
$(document).on('click','#send-btn', function() {
    ...
    if (roomid == -1 || userid == -1){
        updateAppStatusArea("You are not allowed to start chat before you selecting room and user", true);
        // var statusArea = document.getElementById('status-textarea');
